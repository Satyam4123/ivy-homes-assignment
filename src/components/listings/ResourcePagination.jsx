function ResourcePagination({ label, offset, count, total, has_more: hasMore, onPrevious, onNext, isLoading }) {
    ...
    return (
        <div className="mt-8 flex flex-col gap-4 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-500">Showing {firstResult}-{lastResult} of {total} {label}</p>
            <div className="flex gap-2">
                ...
                <button type="button" onClick={onNext} disabled={!hasMore || isLoading} className="border border-teal-800 px-4 py-2 text-sm font-semibold text-teal-800 hover:bg-teal-800 hover:text-white disabled:cursor-not-allowed disabled:opacity-40">Next</button>
            </div>
        </div>
    )
}
