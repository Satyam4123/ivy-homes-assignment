...
function ProjectCard({ project }) {
    return (
        <article className="flex h-full flex-col border border-slate-200 bg-white p-5 transition hover:border-teal-300 hover:shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-auto pt-5">{project.project_url ? <a href={project.project_url} target="_blank" rel="noreferrer" className="block w-full border border-teal-800 px-4 py-2.5 text-center text-sm font-semibold text-teal-800 transition hover:bg-teal-800 hover:text-white">View project</a> : <p className="text-sm text-slate-500">Project link unavailable.</p>}</div>
        </article>
    )
}
